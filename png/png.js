document.addEventListener('DOMContentLoaded', () => {
    ...
    // 変数定義エラー防止のため冒頭で取得
    const targetMenu = sideMenuDrawer || sideMenuPanel;

    // 【修正点1】描画に必要な最小限の要素のみを必須判定とし、部分的なDOM不足でアプリ全体が停止するのを防止
    if (!canvas || !ctx || !imageLoader) {
        console.error("Critical DOM elements (canvas or imageLoader) are missing. Check your HTML structure.");
        return;
    }
    ...
    let brushSize = brushSizeInput ? parseInt(brushSizeInput.value) : 20;
    ...
    // ----------------------------------------------------
    // イベントリスナーの初期登録関数（ライフサイクルを通じて1回のみ実行）
    // ----------------------------------------------------
    ...
    const closeMenu = () => {
        if (targetMenu && !targetMenu.classList.contains('hidden')) {
            ...
            if (menuToggleBtn) menuToggleBtn.textContent = 'メニュー ☰';
        }
    };
    ...
    imageLoader.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                // 【修正点2】canvas.parentElement の null チェックを強化（フォールバック処理を追加）
                const container = canvas.parentElement;
                const scale = container 
                    ? Math.min(container.clientWidth / targetWidth, container.clientHeight / targetHeight, 1.0)
                    : 1.0;
    ...
    const updateSamplerIndicator = (x, y, color) => {
        if (!colorSamplerIndicator) return;
    ...
    // 【修正点3】 deactivationsamplerMode からの initCanvasEventListeners() 重複呼び出しを削除
    const deactivateSamplerMode = () => {
        if (samplerActive) {
            ...
            window.removeEventListener('touchmove', handleSamplerMove);
        ...
        try {
            if (typeof bodyPix !== 'undefined') {
                try {
                    ...
                    console.log("BodyPix ResNet50 model loaded successfully.");
                } catch (e) {
                    console.warn("ResNet50 load failed, falling back to MobileNetV1", e);
                    bodyPixNet = await bodyPix.load({
                    ...
                    console.log("BodyPix MobileNetV1 model loaded successfully.");
                }
            }
        } catch (error) {
    ...
    const applyAutoBackgroundRemoval = async () => {
        ...
        try {
            const MAX_PROCESS_SIZE = 1000;
            let processWidth = canvas.width;
            let processHeight = canvas.height;

            if (processWidth > MAX_PROCESS_SIZE || processHeight > MAX_PROCESS_SIZE) {
                if (processWidth > processHeight) {
                    processHeight = Math.round((MAX_PROCESS_SIZE / processWidth) * processHeight);
                    processWidth = MAX_PROCESS_SIZE;
                } else {
                    processWidth = Math.round((MAX_PROCESS_SIZE / processHeight) * processWidth);
                    processHeight = MAX_PROCESS_SIZE;
                }
            }

            const procCanvas = document.createElement('canvas');
            procCanvas.width = processWidth;
            procCanvas.height = processHeight;
            const procCtx = procCanvas.getContext('2d');
            procCtx.drawImage(canvas, 0, 0, processWidth, processHeight);

            const segmentation = await bodyPixNet.segmentPerson(procCanvas, {
            ...
            const scaleX = processWidth / canvas.width;
            const scaleY = processHeight / canvas.height;

            for (let y = 0; y < canvas.height; y++) {
                for (let x = 0; x < canvas.width; x++) {
                    const procX = Math.floor(x * scaleX);
                    const procY = Math.floor(y * scaleY);
                    const procIndex = procY * processWidth + procX;

                    if (segmentationData[procIndex] === 0) { 
    ...
    const applyAutoBackgroundRemoval = async () => {
        ...
        try {
            // スマホなどの高解像度画像用処理: 解析用の一時Canvasに縮小描画して認識精度を安定させる
            const MAX_PROCESS_SIZE = 1000;
            let processWidth = canvas.width;
            let processHeight = canvas.height;
            ...
            // 縮小処理を行った場合は元の画像サイズに座標変換してマスク適用
            const scaleX = processWidth / canvas.width;
            const scaleY = processHeight / canvas.height;
